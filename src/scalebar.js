        ...
        getScalebarLocation: function() {
            ...
            if (this.location === $.ScalebarLocation.TOP_LEFT) {
                ...
                if (this.stayInsideImage) {
                    var pixelTL = this.viewer.viewport.pixelFromPoint(
                            new $.Point(0, 0), true);
                    if (!this.viewer.wrapHorizontal) {
                        xTL = Math.max(pixelTL.x, 0);
                    }
                    if (!this.viewer.wrapVertical) {
                        yTL = Math.max(pixelTL.y, 0);
                    }
                }
                return new $.Point(xTL + this.xOffset, yTL + this.yOffset);
            }
            if (this.location === $.ScalebarLocation.TOP_RIGHT) {
                var xTR = container.offsetWidth - barWidth;
                var yTR = 0;
                if (this.stayInsideImage) {
                    var pixelTR = this.viewer.viewport.pixelFromPoint(
                            new $.Point(1, 0), true);
                    if (!this.viewer.wrapHorizontal) {
                        xTR = Math.min(xTR, pixelTR.x - barWidth);
                    }
                    if (!this.viewer.wrapVertical) {
                        yTR = Math.max(yTR, pixelTR.y);
                    }
                }
                return new $.Point(xTR - this.xOffset, yTR + this.yOffset);
            }
            if (this.location === $.ScalebarLocation.BOTTOM_RIGHT) {
                var xBR = container.offsetWidth - barWidth;
                var yBR = container.offsetHeight - barHeight;
                if (this.stayInsideImage) {
                    var pixelBR = this.viewer.viewport.pixelFromPoint(
                            new $.Point(1, 1 / this.viewer.source.aspectRatio),
                            true);
                    if (!this.viewer.wrapHorizontal) {
                        xBR = Math.min(xBR, pixelBR.x - barWidth);
                    }
                    if (!this.viewer.wrapVertical) {
                        yBR = Math.min(yBR, pixelBR.y - barHeight);
                    }
                }
                return new $.Point(xBR - this.xOffset, yBR - this.yOffset);
            }
            if (this.location === $.ScalebarLocation.BOTTOM_LEFT) {
                var xBL = 0;
                var yBL = container.offsetHeight - barHeight;
                if (this.stayInsideImage) {
                    var pixelBL = this.viewer.viewport.pixelFromPoint(
                            new $.Point(0, 1 / this.viewer.source.aspectRatio),
                            true);
                    if (!this.viewer.wrapHorizontal) {
                        xBL = Math.max(xBL, pixelBL.x);
                    }
                    if (!this.viewer.wrapVertical) {
                        yBL = Math.min(yBL, pixelBL.y - barHeight);
                    }
                }
                return new $.Point(xBL + this.xOffset, yBL - this.yOffset);
            }
